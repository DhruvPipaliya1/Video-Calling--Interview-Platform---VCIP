...
export async function createSession(req, res) {
    try {
        ...
        if(!problem || !difficulty){
            return res.status(400).json({ message: "Problem and dficulty are required" })
        }
        
        //generate a unque call id for stream video
...
export async function getMyRecentSessions(req, res) {
    try {
        ...
        console.log("Error in getMyRecentSessions controller:", error.messages);
...
export async function getSessionById(req, res) {
    try {
        ...
        .populates("host", "name email profileImage clerkId")
